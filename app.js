...
//     cleanFieldRef.addEventListener("input", () => {
//       clearTimeout(recordSearchTimer);
//       recordSearchTimer = setTimeout(() => renderRecords(main), 400);
//     });
//   }
// }
...
//           <div class="result-meta">Encounter Reference Date: ${caseDate}</div>
//         </div>`;
//       }).join("") + `</div>`;
//     }
...
//       </div>
//       <div class="filters">
//         <div class="filter-chip active">Last 1 Month</div>
//       </div>
...
//       caseSearchTimer = setTimeout(() => renderCases(main), 400);
//     });
//   }
// }
...
// // ─────────────────────────────────────────────
// async function renderRecords(main) {
//   // If the shell (search bar + filters) isn't rendered yet, build it first
//   if (!document.getElementById("recordSearch")) {
//     main.innerHTML = `
//       <div class="search-wrapper">
//         <span class="search-icon">🔍</span>
//         <input type="text" class="form-input search-input"
//           placeholder="Search by Patient Name, Patient ID or Chief Complaint..."
//           id="recordSearch" value="">
//       </div>
//       <div class="filters">
//         <div class="filter-chip ${activeRecordFilter === 'all' ? 'active' : ''}" id="filterChipAll" onclick="setRecordFilter('all')">My Patients</div>
//         <div class="filter-chip ${activeRecordFilter === 'recent' ? 'active' : ''}" id="filterChipRecent" onclick="setRecordFilter('recent')">Today's Visits</div>
//       </div>
...
//       clearTimeout(recordSearchTimer);
//       recordSearchTimer = setTimeout(() => updateRecordResults(), 400);
//     });
//   }

//   // Update filter chip active states
//   const chipAll = document.getElementById("filterChipAll");
//   const chipRecent = document.getElementById("filterChipRecent");
//   if (chipAll) chipAll.className = `filter-chip ${activeRecordFilter === 'all' ? 'active' : ''}`;
//   if (chipRecent) chipRecent.className = `filter-chip ${activeRecordFilter === 'recent' ? 'active' : ''}`;

//   await updateRecordResults();
// }

// async function updateRecordResults() {
//   const searchInput = document.getElementById("recordSearch");
//   const query = searchInput ? searchInput.value.trim() : "";
...
//   if (!document.getElementById("caseSearch")) {
//     main.innerHTML = `
//       <div class="search-wrapper">
//         <span class="search-icon">🔍</span>
//         <input type="text" class="form-input search-input"
...
//         <div class="filter-chip active">Last 1 Month</div>
//       </div>
//       <div id="caseCountLabel" style="margin:16px 0; font-size:15px; font-weight:600;"></div>
...
//     searchBox.addEventListener("input", () => {
//       clearTimeout(caseSearchTimer);
//       caseSearchTimer = setTimeout(() => updateCaseResults(), 400);
//     });
//   }
...
// async function updateCaseResults() {
//   const caseSearchInput = document.getElementById("caseSearch");
//   const query = caseSearchInput ? caseSearchInput.value.trim() : "";
//   const resultsDiv = document.getElementById("caseResults");
//   const countLabel = document.getElementById("caseCountLabel");
//   if (!resultsDiv) return;
...
//         (c.clinicalNotes && c.clinicalNotes.toLowerCase().includes(q)) ||
//         (c.visitType && c.visitType.toLowerCase().includes(q)) ||
//         (c.patientId && c.patientId.toLowerCase().includes(q))
//       );
//     }

//     if (countLabel) countLabel.textContent = `${matchingCases.length} similar cases discovered in this timeline`;
...
//   resultsDiv.innerHTML = caseListHTML;
// }
...
// Search state — stored outside render so typing doesn't reset
  ...
  if (currentStep === 1) {
    ...
    content = `
      <div class="content-box">
        <h2 class="content-title">Patient consultation — step 3</h2>
        <div class="upload-area" id="uploadArea">
          <input type="file" id="fileInput" multiple accept=".pdf,.jpg,.jpeg,.png,.webp,.dcm" style="display:none">
          <div class="upload-icon">📄</div>
          <div class="upload-text">Click to upload MRI scans / reports</div>
          <div class="upload-hint">PDF, JPG, PNG, DICOM supported</div>
        </div>
    ...
    fileInput.addEventListener("change", async (e) => {
      ...
      for (const file of selectedFiles) {
        try {
          const extMap = {
            jpg: "image/jpeg", jpeg: "image/jpeg", png: "image/png",
            webp: "image/webp", pdf: "application/pdf", dcm: "application/dicom"
          };
          const ext = file.name.split('.').pop().toLowerCase();
          const mimeType = file.type || extMap[ext] || "application/octet-stream";
...
function displayFilesList() {
  ...
  filesWrapper.innerHTML = formData.files.map(file => {
    const iconMap = { jpg: "🖼️", jpeg: "🖼️", png: "🖼️", webp: "🖼️", pdf: "📄", dcm: "🩻" };
    const ext = file.name.split('.').pop().toLowerCase();
    const icon = iconMap[ext] || "📎";
    const statusColor = file.error ? "#ef4444" : "#10b981";
    const statusText = file.error ? `❌ Upload failed — ${file.errorMsg || "Check console for details"}` : "✅ Uploaded to S3";
...
async function renderRecords(main) {
  ...
  try {
    ...
    // Filter to only this doctor's records
    let records = allRecords.filter(r => {
      const recordDocId = r.doctorId || r.doctor || "";
      const currentDocId = loggedInDoctor ? (loggedInDoctor.id || loggedInDoctor.doctorId) : "";
      return String(recordDocId).trim() == String(currentDocId).trim();
    ...
    // Apply search filter using stored query
    ...
    if (!records || records.length === 0) {
      ...
      recordsHTML = `<div class="results">` + records.map(p => {
        ...
        if (p.files && p.files.length > 0) {
          documentsBadgeHTML = p.files.map(f => {
            if (!f.key && !f.fileKey && !f.url) return "";
            const rawKey = f.key || f.fileKey || f.url.replace(`https://doctor-portal-files-25.s3.ap-south-1.amazonaws.com/`, "");
            return `
              <span class="tag"
                style="background:var(--teal-light); color:var(--teal-dark); border-color:var(--teal); cursor:help; user-select:none;"
                onmouseenter="fetchAndShowPreview(event, '${rawKey.replace(/'/g, "\\'")}', '${f.name.replace(/'/g, "\\'")}')"
  ...
  main.innerHTML = `
    <div class="search-wrapper">
      ...
      <input type="text" class="form-input search-input"
        placeholder="Search by Patient Name, Patient ID or Chief Complaint..."
        id="recordSearch" value="${recordSearchQuery}">
    </div>
    <div class="filters">
      <div class="filter-chip ${activeRecordFilter === 'all' ? 'active' : ''}" onclick="setRecordFilter('all')">My Patients</div>
      <div class="filter-chip ${activeRecordFilter === 'recent' ? 'active' : ''}" onclick="setRecordFilter('recent')">Today's Visits</div>
  ...
  if (cleanFieldRef) {
    ...
    cleanFieldRef.setSelectionRange(cleanFieldRef.value.length, cleanFieldRef.value.length);
    let recordSearchTimer;
    cleanFieldRef.addEventListener("input", (e) => {
      recordSearchQuery = e.target.value;
      clearTimeout(recordSearchTimer);
      recordSearchTimer = setTimeout(() => renderRecords(main), 500);
    });
  }
}
...
async function renderCases(main) {
  ...
  try {
    ...
    if (!matchingCases || matchingCases.length === 0) {
      ...
      caseListHTML = `<div class="results">` + matchingCases.map((c, idx) => {
        const caseId = c.patientId || c.id || "CASE-REF";
        const caseTitle = c.chiefComplaint || "General Health Consultation";
        const caseNotes = c.clinicalNotes || "No notes mapped.";
        const caseDate = c.visitDate || c.date || "N/A";
        const visitType = c.visitType || "Internal Case";

        let baseMatch = 100 - (idx * 4);
        if (caseSearchQuery && !caseTitle.toLowerCase().includes(caseSearchQuery.toLowerCase())) baseMatch -= 7;
        const finalMatchRate = Math.max(74, baseMatch);
    ...
    main.innerHTML = `
      <div class="search-wrapper">
        ...
        <input type="text" class="form-input search-input"
          placeholder="Search by symptoms, diagnosis, ICD code..."
          id="caseSearch" value="${caseSearchQuery}">
      </div>
      <div class="filters">
        <div class="filter-chip active">Last 1 Month</div>
      </div>
  ...
  if (searchBox) {
    ...
    searchBox.addEventListener("input", (e) => {
      ...
      clearTimeout(caseSearchTimer);
      caseSearchTimer = setTimeout(() => renderCases(main), 500);
    });
  }
}
